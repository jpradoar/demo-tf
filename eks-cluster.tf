...
resource "aws_iam_role" "demo-cluster" {
  name = "iam-role-eks-${var.cluster-name}"
...
resource "aws_security_group" "demo-cluster" {
  ...
  tags = {
    Name    = "sg-${var.vpc_name}-${var.cluster-name}"
    part_of = "eks"
  }
}
...
resource "aws_eks_cluster" "demo" {
  ...
  version  = var.k8s-version          # I prefere mantain same version for both
